    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        defer {
            DispatchQueue.main.sync {
                self.processPoints(ringPip: ringPip, ringMcp: ringMcp)
            }
        }
        ...
        do {
            ...
            // Look for tip points.
            guard let piPPoint = ringFingerPoints[.ringPIP], let mcpPOints = ringFingerPoints[.ringMCP] else {
                return
            }
            // Ignore low confidence points.
            guard piPPoint.confidence > 0.3 && mcpPOints.confidence > 0.3 else {
                return
            }
            // Convert points from Vision coordinates to AVFoundation coordinates.
            ringPip = CGPoint(x: piPPoint.location.x, y: 1 - piPPoint.location.y)
            ringMcp = CGPoint(x: mcpPOints.location.x, y: 1 - mcpPOints.location.y)
        } catch {
            cameraFeedSession?.stopRunning()
